...
class SecurityProxyMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Bypass public, metrics, and documentation routes
        public_paths = {
            "/metrics",
            "/metrics/",
            "/api/v1/health",
        ...
        if request.url.path in public_paths or request.url.path.startswith("/docs") or request.url.path.startswith("/metrics"):
